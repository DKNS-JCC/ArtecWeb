            ...
            <Card v-for="robot in robots" :key="robot.id"
                ...
                <template v-if="isPlatformAdmin">
                    <div class="flex items-center gap-4 mb-6">
                        <div
                            class="w-12 h-12 bg-primary/10 rounded-sm flex items-center justify-center text-primary flex-shrink-0">
                            <Bot class="w-6 h-6" />
                        </div>
                        <div class="min-w-0">
                            <h2 class="font-display text-xl font-medium tracking-tight text-foreground truncate">{{
                                robot.name }}</h2>
                    ...
                    <div class="flex items-center gap-2 mb-6 text-sm">
                        ...
                        <span v-if="robot.museum_id" class="font-medium text-foreground">{{ museumName(robot.museum_id)
                            }}</span>
                    ...
                    <div class="flex gap-2 pt-4 border-t border-border">
                        ...
                        <Button variant="ghost" size="sm" class="gap-1.5 text-destructive hover:text-destructive"
                ...
                <template v-else>
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h2 class="font-display text-xl font-medium tracking-tight text-foreground">{{ robot.name }}
                            </h2>
                            <span class="text-xs text-muted-foreground uppercase tracking-wider">{{ robot.id }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="relative flex h-3 w-3">
                                <span v-if="robot.connected"
                                    class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                                <span class="relative inline-flex rounded-full h-3 w-3"
                                    :class="robot.connected ? 'bg-green-500' : 'bg-red-500'"></span>
                            </span>
                    ...
                    <div class="space-y-3 mb-6">
                        ...
                        <div class="flex justify-between items-center text-sm">
                            ...
                            <span v-if="robot.connected"
                                class="font-mono bg-secondary px-2 py-0.5 rounded-sm text-xs text-secondary-foreground">
                        ...
                        <div v-if="robot.connected && robot.current_location"
                        ...
                        <div v-if="robot.last_nav_error_at"
                            class="flex items-start gap-2 text-sm rounded-sm bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700/40 px-2.5 py-2">
                            ...
                            <span class="text-xs text-red-700 dark:text-red-300 leading-snug">
                                No pudo llegar<template v-if="robot.last_nav_error_place"> a <strong>{{
                                        robot.last_nav_error_place }}</strong></template>.
                                <button @click="$emit('show-incidents')"
                                    class="underline font-semibold whitespace-nowrap">Ver incidencias</button>
                            </span>
                        </div>
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-muted-foreground flex items-center gap-1">
                            ...
                            <div v-if="robot.is_occupied" class="flex items-center gap-2">
                                ...
                                <Button v-if="isMuseumAdmin" @click="$emit('end-visit', robot.id)" variant="destructive"
                                    size="icon" class="h-6 w-6 scale-90" title="Finalizar Visita Forzosamente">
                                    <X class="w-3.5 h-3.5" />
                                </Button>
                            </div>
                        ...
                        <div class="flex justify-between items-center text-sm pt-2 border-t border-border/50">
                            ...
                            <div class="flex items-center gap-1">
                                <span
                                    :class="robot.connected ? 'text-green-700 dark:text-green-400 bg-green-500/10' : 'text-red-700 dark:text-red-400 bg-red-500/10'"
                    ...
                    <div class="border-t border-border pt-4 flex flex-col gap-2">
                        <div class="flex gap-2">
                            <Button v-if="!robot.connected" @click="$emit('command', robot.id, 'connect')"
                                :disabled="pendingCommandId === robot.id" size="sm"
                            ...
                            <Button v-else @click="$emit('command', robot.id, 'disconnect')"
                                :disabled="pendingCommandId === robot.id" size="sm" variant="destructive"
                                class="flex-1 gap-1.5">
                                <Loader2 v-if="pendingCommandId === robot.id" class="w-4 h-4 animate-spin" />
                                {{ pendingCommandId === robot.id ? 'Desconectando…' : 'Desconectar' }}
                            </Button>
                        </div>
                        <Button v-if="isStaff" @click="$emit('open-control', robot.id)" variant="outline" size="sm"
                            class="gap-1.5">
                            <Gamepad2 class="w-4 h-4" /> Control
                        </Button>
                        <Button v-if="isStaff" @click="$emit('edit', robot)" variant="outline" size="sm"
                            class="gap-1.5">
                    ...
                    <div class="mt-4 pt-4 border-t border-border flex flex-col gap-2">
                        <img v-if="qrCodes[robot.id]" :src="qrCodes[robot.id]" alt="QR Code"
                            class="w-24 h-24 mx-auto" />
                        <div v-else
                            class="w-24 h-24 mx-auto flex items-center justify-center text-xs text-muted-foreground">
                            Generando QR…
                        </div>
                        <Button v-if="qrCodes[robot.id]" as="a" :href="qrCodes[robot.id]"
                            :download="`qr-${robot.name || robot.id}.png`" size="sm" class="mt-2">Descargar</Button>

                        <Button as="a" :href="buildVisitUrl(robot.id)" target="_blank" variant="outline" size="sm"
                            class="mt-2">Chat</Button>
                    </div>
                </template>
            </Card>
        </div>
